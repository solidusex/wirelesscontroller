...
BOOL CShortcut::CreateShortCut(CString LnkTarget, CString LnkName, UINT SpecialFolder, CString LnkDescription, CString IconLocation, UINT IconIndex)
{
		...
		sSpecialFolder += LnkName + TEXT(".") + TEXT("lnk");

		if(LnkTarget == TEXT("_this"))
		{
				cfFull.SetFilePath(sExePath);
				sExe = cfFull.GetFileName();
				sExe.Delete(sExe.Find(TEXT(".")) + 1, 3);
		}
		else
		{
				sExePath = LnkTarget;
		...
		if (SUCCEEDED( CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER, IID_IShellLink,(LPVOID*) &psl)))
		{
				...
				if (SUCCEEDED(psl->QueryInterface(IID_IPersistFile, (LPVOID *)&ppf)))
				{
						...
						//MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, sSpecialFolder, -1, wsz,			MAX_PATH);
...
#ifdef _DEBUG
								if(FAILED(hr))
										TRACE(TEXT("IconLocation not changed!\n"));
#endif
						}
...
BOOL CShortcut::DeleteShortCut(CString LnkName, UINT SpecialFolder)
{
		...
		if(sSpecialFolder.Find(TEXT('\0'))!=sSpecialFolder.GetLength())
		{
				FIO.fFlags|=FOF_MULTIDESTFILES;
		}
		if(sSpecialFolder.Right(1))
		{
				sSpecialFolder+= TEXT('\0');
		}
		...
		if(!bD)
		{
				TRACE(TEXT("Lnk Deleted!\n"));
				return TRUE;
		}
		else
		{
				TRACE(TEXT("Lnk NOT Deleted! DeleteShortCut(...) FAILED!\n"));
				return FALSE;
		}
}
...
BOOL CShortcut::isLinkAvailable(CString LnkName, UINT SpecialFolder)
{
		...
		{
#ifdef _DEBUG
				afxDump << TEXT("Full file name = ") << cfStatus.m_szFullName << TEXT("\n");
#endif
				return TRUE;
		}
		else
		{
#ifdef _DEBUG
				afxDump << TEXT("File NOT available = ") << cfStatus.m_szFullName << TEXT("\n");
#endif
				return FALSE;
		}
}
...
HRESULT CShortcut::ResolveLink(CString LnkName, UINT SpecialFolder, HWND hwnd, CString &LnkPath, CString &LnkDescription)
{
		...
		// build a linkfile:
		sLnkFile = sSpecialFolder + LnkName + TEXT(".lnk");
